...
# N.B Text is always argument #0, title argument #2, options argument#3
# and preselect argument 4
title = "UWS STUDY BUDDY"

# global variable whose keys hold existing request IDs; values are not
# defined (None)
requests = {}
...
class RequestGUI(Tk):
    def __init__(self, window, rid, form=None):
        ...
        # , show=bullet)
        self.T3 = Entry(window, textvariable=form["password"])
        # , show=bullet)
        self.T4 = Entry(window, textvariable=form["password2"])
        ...
        form["password"].trace("w", lambda name, index,
                               mode, sv=form["password"]: hidepw(self.T3))
        form["password2"].trace("w", lambda name, index,
                                mode, sv=form["password2"]: hidepw(self.T4))
        ...
        form["mod"] = StringVar()
        form["year"] = StringVar()
        self.rx1 = ["Campus Location", "Ayr",
                    "Dumfries", "Paisley", "Hamilton"]
        ...
        for i in range(7):
            ...
            for j in range(3):
                ...
                self.timeBoxes[i].append(Checkbutton(
                    window, variable=form["timeVars"][i][j]))
...
class Request:
    def __init__(self, requestid):
        ...
        root.mainloop()
        msg = "Enter Your Request Information"
        title = "Request Info"
        ...
        # adds this request ID to the (unsaved) list of requests
        requests[requestid] = None
...
def generate_Random():
    ...
    while newId == 0 or newId in requests:  # reject candidate ID if invalid or already exists
        newId = randrange(1e5, 1e6)  # generate a candidate 6-digit number
    return newId
